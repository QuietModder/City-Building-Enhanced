...
//		riverWidthController = 0; 	// for testing
	...
	// ============= CBE Expanded Sea Configuration =============
	// This copy intentionally chooses only water-focused layouts already supported
	// by the original Unknown generator.
	float landConfig = rmRandFloat(0,1);
	if (rmGetIsTreaty() == true)
	{
		// Avoid the split-island branch for Treaty, matching the original script's
		// conservative handling of that mode.
		if (landConfig < 0.60)
			oceanRing = 1;          // big island
		else
			oceanOffCenter = 1;    // Saguenay-style sea
	}
	else
	{
		if (landConfig < 0.40)
			oceanRing = 1;          // big island
		else if (landConfig < 0.70)
			splitIsland = 1;        // Caribbean-style split islands
		else if (landConfig < 0.90)
			oceanOffCenter = 1;    // Saguenay-style sea
		else
		{
			oceanMiddle = 1;        // great-lake / central-water layout
			blockedMiddle = 1;
		}
	}
	rmEchoInfo("CBE layout = sea map");
...
//		rmSetAreaBaseHeight(flagPondID4, 01);	// for testing
	...
	numTries=
	if (floodedLand == 1)
		numTries=60+45*cNumberNonGaiaPlayers;
	failCount = 0;
